...
const userDeletedPostModel = mongoose.model(
  "deletedpost",
  userDeletedPostSchema
);
...
app.post("/login", (request, response) => {
  ...
  let found = userModel.find({ username: usernameee }, (err, result) => {
    if (err) {
      ...
      console.log("Nothing");
      response.send({
        message: `Dear ${usernameee}, you do not have an account here...`,
        result,
      });
    } else if (result) {
      console.log(result);
      ...
      bcryptjs
        ...
        .then((result) => {
          ...
          if (result == true) {
            ...
            response.send({
              message:
                "I don't know what's up, but your password is definitely wrong!",
              result,
            });
          }
        });
...
app.get("/deletedposts", (request, response) => {
userDeletedPostModel.find((err, result) => {
  if (err) {
    console.log(err.message);
  } else {
    response.send(result);
...
app.post("/changeprofile", (request, response) => {
  ...
  found = userProfileModel.findOne(
    { username: request.body.username },
    (err, result) => {
      if (result) {
        console.log(result);
